...
def compute_gp(D, real_samples, fake_samples):
    ...
    - In WGAN, the discriminator (critic) must be 1-Lipschitz continuous to compute the 
      Wasserstein distance correctly. This means the gradient norm ||∇D(x)|| should be ≤ 1 
    ...
    The penalty term: LAMBDA_GP * mean((||∇D(x̂)||₂ - 1)²)
    where x̂ = α * x_real + (1 - α) * x_fake, α ~ Uniform(0,1)
    
    This encourages the discriminator to have gradients with norm ≈ 1 everywhere along
    the path between real and fake samples, ensuring stable training.
    """
    # Step 1: Create random interpolation coefficients α ~ Uniform(0,1)
    # Each sample in the batch gets its own random α value
    ...
    # Step 2: Create interpolated samples x̂ = α * x_real + (1 - α) * x_fake
    ...
    # We use autograd.grad to get ∇D(x̂), the gradient of the discriminator at each interpolated point
    ...
    # Step 6: Compute penalty = LAMBDA_GP * mean((||∇D(x̂)||₂ - 1)²)
    ...
    gradient_norms = gradients.norm(2, dim=1)  # L2 norm per sample: ||∇D(x̂)||₂
...
def train_wgan_gp(loader, G, D, device, noise_dim, feature_cols, start_epoch=0, previous_g_losses=None, previous_d_losses=None):
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        if is_warmup and epoch == start_epoch:
            print(f"🔥 Discriminator warm-up phase: Training discriminator only for {DISC_WARMUP_EPOCHS} epochs")
        
        if not is_warmup and epoch == DISC_WARMUP_EPOCHS:
            print(f"✅ Warm-up complete! Starting generator training from epoch {DISC_WARMUP_EPOCHS + 1}")
        ...
        if gap_signed < 0:
            ...
            if current_d_lr < 2e-5:  # Only increase if not too high
                ...
                d_lr_adjusted = True
                print(f"  ⚠️  Discriminator backwards (D(real)={d_real_val:.4f} < D(fake)={d_fake_val:.4f}), increasing D LR from {current_d_lr:.8f} to {new_d_lr:.8f}")
            ...
            if not is_warmup and current_g_lr > 1e-5:
                ...
                g_lr_adjusted = True
                print(f"  ⚠️  Discriminator backwards, reducing G LR from {current_g_lr:.8f} to {new_g_lr:.8f}")
            ...
            if current_d_lr > 1e-8:
                ...
                for param_group in opt_D.param_groups:
                    param_group['lr'] = new_d_lr
                d_lr_adjusted = True
                print(f"  ⚠️  Gap too large ({gap:.4f}, D(real)={d_real_val:.4f} > D(fake)={d_fake_val:.4f}), reducing D LR from {current_d_lr:.8f} to {new_d_lr:.8f}")
            ...
            if not is_warmup and gap > GAP_THRESHOLD * 1.5 and current_g_lr < 5e-5:
                ...
                for param_group in opt_G.param_groups:
                    param_group['lr'] = new_g_lr
                g_lr_adjusted = True
                print(f"  ⚠️  Gap very large ({gap:.4f}), increasing G LR from {current_g_lr:.8f} to {new_g_lr:.8f}")
